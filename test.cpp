...
std::map<std::pair<uint8_t, uint8_t>, ExecuteFunc> ExecuteFuncMap = {
    ...
    {{0x13, 0},  Execute_addi},
    {{0x13, 2},  Execute_slti},
    {{0x13, 3},  Execute_sltiu},
    {{0x13, 4},  Execute_xori},
    {{0x13, 6},  Execute_ori},
    {{0x13, 7},  Execute_andi},
    {{0x13, 1},  Execute_slli},
    {{0x13, 5},  Execute_srli},
    {{0x13, 13}, Execute_srai},
    {{0x33, 0},  Execute_add},
    {{0x33, 8},  Execute_sub},
    {{0x33, 1},  Execute_sll},
    {{0x33, 2},  Execute_slt},
...
class RV32IInterpreter {
  ...
  void PrintRegisters() {
    for (int i = 0; i < 32; i++) {
      DEBUG_CERR << std::dec << i << "\t" << reg[i] << std::endl;
    }
  }
 ...
 public:
  ...
  void RunProgram() {
    ...
    while (Fetch()) {
      ...
      std::cout << std::dec << counter << ": " << std::hex << PC << std::endl;
      if (print_reg) {
        PrintRegisters();
        std::cout << std::endl;
      }
...
int main() {
  ...
  if (print_reg) {
    ...
    freopen("pc_answer.txt", "w", stdout);
  }
